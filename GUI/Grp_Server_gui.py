...
def initialize_client():
    ...
    while True:
        ...
        rsa = ds["rsa"]
        info[addr] = [name,rsa]
        s_in = ','.join(info[i][0] for i in addresses if i != addr)
        if len(addresses)>1:
        ...
        data_to_send = {'pub': public_key, 'key':RSA.RSA_encrypt(session_key,rsa),'s_info':se_info,'info':s_in}
...
def GUI():
    ...
    button_image_2 = PhotoImage(file="picture4.png")
    button_2 = Button(image=button_image_2,borderwidth=0,highlightthickness=0,command=lambda: transistion() ,relief="flat")
    button_2.place(x=scale_x*511.0,y=scale_y*500.0,width=470.0,height=65.0)
    
    canvas.create_text(scale_x*620.0,scale_y*400.0,anchor="nw",text="Server Running",fill="#FFFFFF",font=("RobotoRoman ExtraBold", 35 * -1))

    

    image_image_1 = PhotoImage(file="image_1.png")
    image_1 = canvas.create_image(scale_x*750.0,scale_y*196.0,image=image_image_1)
    _thread.start_new_thread(initialize_client, ())
    window.mainloop()
